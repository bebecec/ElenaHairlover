  ...
  return (
    <section className="py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-100 space-y-6">
          ...
          <div className="space-y-4 border-b border-slate-100 pb-6">
            <div className="flex items-start space-x-3">
              ...
              <div>
                <h3 className="font-bold text-slate-900 text-base">{salon.name}</h3>
                <p className="text-xs text-slate-600 mt-0.5">{salon.address}</p>
                <p className="text-xs text-slate-600">{salon.postalCode} - {salon.city}</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3 pt-2">
              <a
                ...
                className="inline-flex items-center px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 font-bold text-xs transition-colors"
              >
              ...
              <a
                ...
                className="inline-flex items-center px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs transition-colors shadow-xs"
              >
          ...
          <div>
            ...
            <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-2 text-xs">
              {salon.openingHours.map((oh) => (
                <div key={oh.day} className="flex justify-between items-center py-1 border-b border-slate-200/60 last:border-0">
                  <span className="font-bold text-slate-700">{oh.day}</span>
                  <span className={oh.isOpen ? 'font-bold text-slate-900' : 'font-semibold text-rose-500'}>
                    {oh.hours}
                  </span>
                </div>
              ))}
          ...
          {/* Cancellation Policy */}
          <div className="bg-rose-50/70 p-4 rounded-2xl border border-rose-100 text-xs text-rose-950 space-y-1">
          ...
          <div className="flex items-center space-x-4 pt-2 border-t border-slate-100 text-xs">
            <span className="font-bold text-slate-500">Síguenos:</span>
            <a
              href={salon.instagram}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center text-slate-700 hover:text-rose-600 font-semibold"
            >
              <Instagram className="w-4 h-4 mr-1 text-rose-500" />
              Instagram
            </a>
            <a
              href={salon.facebook}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center text-slate-700 hover:text-blue-600 font-semibold"
            >
        ...
        {/* Google Map Frame */}
        <div className="bg-white rounded-3xl p-4 shadow-sm border border-slate-100 flex flex-col justify-between overflow-hidden">
          ...
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 mt-4 text-xs text-slate-600 flex items-center justify-between">
            <div>
              <p className="font-bold text-slate-900">¿Cómo llegar?</p>
